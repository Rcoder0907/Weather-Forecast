...
import { WeatherForecastAPI } from "../helpers/API";
...
export const getWeatherByLocation = (toast) => (dispatch) => {
    ...
    const success = async (position) => {
        try {
            ...
            let weatherData = await axios.get(`/weather?lat=${latitude}&lon=${longitude}&appid=${WeatherForecastAPI}`);
            weatherData = weatherData.data;
            let forcastData = await axios.get(`/onecall?lat=${latitude}&lon=${longitude}&exclude=hourly,minutely&units=metric&appid=${WeatherForecastAPI}`);
...
export const getWeatherByCity = (city, toast) => async (dispatch) => {
    try {
        dispatch(getDataLoading());
        let weatherData = await axios.get(`/weather?q=${city}&appid=${WeatherForecastAPI}`);
        weatherData = weatherData.data;
        let { lon, lat } = weatherData.coord;
        let forcastData = await axios.get(`/onecall?lat=${lat}&lon=${lon}&exclude=hourly,minutely&units=metric&appid=${WeatherForecastAPI}`);
        forcastData = forcastData.data.daily;
        let payload = { weatherData, forcastData };
        dispatch(getDataSuccess(payload));
        setItem("weather", payload);
